...
%%
%% ---------------------------------------------------------------------------
%% ANONYMITY -- CHECK THE TRACK'S RULE BEFORE SUBMITTING.
%%
%% As it stands the paper is NOT anonymised: author names and affiliations
%% appear on page 1 and in every running head. The acmart submission sample
%% uses [acmsmall,screen,anonymous,review], so if this track is
%% double-anonymous, swap the class line below for the commented one.
%%
%% Adding `anonymous` is NECESSARY BUT NOT SUFFICIENT here. The class hides the
%% author block; it cannot hide the places where the text identifies the authors
%% by referring to its own predecessor. If anonymising, also neutralise every
%% occurrence of "the conference version" (grep finds them) and the naming of
%% AgenticSE '26 in the comment above. The usual phrasing is "an earlier version
%% of this work" with the venue withheld, or a citation to it as third-party
%% work.
%%
%% Note that for a JOURNAL-FIRST submission the predecessor is normally declared
%% on purpose -- eligibility depends on it -- so the named form is likely the
%% correct one. Verify against the track's call rather than assuming either way.
%% ---------------------------------------------------------------------------
%\documentclass[acmsmall,screen,anonymous,review]{acmart}
%% `anonymous` suppresses the author block, affiliations and e-mail addresses
%% for double-anonymous review. Nothing else in this file needs scrubbing: the
%% acknowledgments thank data-source maintainers rather than people or funders,
%% no repository URL appears, and the earlier conference version is referred to
%% without being cited, so the bibliography carries no self-identifying entry.
