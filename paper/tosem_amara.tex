...
One qualification applies to the absolute values in this table and in
Table~\ref{tab:gt-run}, though not to the comparison they support. The
release-fetching path truncated its candidate list to the requested limit
\emph{before} applying the ranking function meant to demote advisory records,
so for any product with an active CVE feed the shipped releases were unreachable:
measured on the repaired code, a query for \texttt{linux} returned no shipped
release at all within forty retrieved documents. Both arms called the same
retriever, so the ordering these tables report is unaffected and the ladder of
Section~\ref{sec:ladder-results}, run after the repair, reaches the same
conclusion. The absolute figures, however, were produced by a retriever that
could not reach part of its corpus, and should be read as a lower bound rather
than as this architecture's ceiling.
...
Section~\ref{sec:ladder} required, and it removes the caveat attached to
Table~\ref{tab:scaled-retrieval}, whose arms queried live endpoints and were
therefore comparable in direction but not paired question-by-question.
