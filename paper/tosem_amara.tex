...
so every adjacent pair is a single-factor comparison. Three rungs carry the
measurements reported in this paper; two are implemented in the released
...
A2 & \texttt{rewrite\_only} & query rewriting & Implemented \\
A3 & \texttt{marag\_llm} & union fetch over both phrasings & Measured \\
A4 & \texttt{marag\_llm\_retry} & Orchestrator retry & Implemented \\
...
We report no measurement for A2, A4, or the fourth cell here. They are
implemented and unit-tested in the released harness, and the ladder is the
protocol we recommend for any subsequent evaluation of this architecture rather
than a result of this one. Two constraints apply to such a run. Because the
retrieval endpoints are live, every arm must execute against a recorded corpus
snapshot in strict replay, or the rungs will differ by corpus drift as well as
by capability. And if the Self-Improvement Memory is ever attached as a further
rung, term-weight learning makes each question's outcome depend on the
questions preceding it; per-question differences then cease to be exchangeable
and the paired tests used throughout this section no longer apply as written.
