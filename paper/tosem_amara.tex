...
Completing that evaluation carries a condition worth stating in advance, because
it is easy to violate by default. The arm's relevance critique discards a
document when it judges it irrelevant, and in the configuration released here
that critique runs the same model that supplies our relevance judgments. Scoring
the arm with that judge would be the circular arrangement of
Section~\ref{sec:related}: an assessor filtering the candidates it will then
grade, which Clarke and Dietz measured collapsing agreement with human judgments
from $\tau \approx 0.84$ to 0.38 at rank~10~\cite{clarke2024cannot}. The arm
would score well by construction rather than by retrieving better. Whoever
finishes the run should therefore judge it with a model independent of its
critic. The harness records each critique decision, so the number of documents
the critic removed is recoverable per query and the inflation can be bounded
rather than assumed; because a hedged or negated critique keeps the document
...
($n=100$), pooled graded relevance, judge \texttt{llama3.1}. The two
multi-agent arms are configured identically and differ only in answer synthesis;
each nonetheless performs its own retrieval against live sources. Their ranked
top-$k$ sets coincide on all 100 questions. Of the 93 questions for which a
candidate pool was recorded, their pools are identical as ordered lists on 88
and as sets on 89. On the remaining 7 every live endpoint returned nothing and
the local fallback corpus supplied the documents, so no pool exists to compare;
those 7 are excluded here rather than counted as agreements. Retrieval is not
frozen for any run reported here: the endpoints are queried live, so figures
from different runs are comparable in direction but not paired
