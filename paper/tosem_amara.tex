...
and the rewritten phrasing and unioning the candidate pools raises nDCG@3 to
0.973 in one step, and on a 100-question benchmark spanning 24 software
ecosystems it reduces the fetch-time loss from 96\% to 14.7\%.
...
retrieval was synthesized through the baseline's own prompt (0.924 versus
...
than beats the single-agent baseline on retrieval (nDCG@3 0.865 versus 0.869,
$n=96$) at roughly twice the latency, so decomposition per se is not what
...
Substring boost & original question & 0.145 & 0.250 & 0.145 \\
Okapi BM25      & original question & 0.212 & 0.500 & 0.203 \\
...
Embedding cosine & original question & \textbf{0.973} & \textbf{1.000} & \textbf{0.933} \\
\bottomrule
\end{tabular}
\end{table}
...
falls from 96\% to 14.7\%, and the repaired pipeline reaches nDCG@3 of 0.865
against the baseline's 0.869 --- parity, within overlapping confidence
intervals, at approximately twice the latency. We report this rather than the
